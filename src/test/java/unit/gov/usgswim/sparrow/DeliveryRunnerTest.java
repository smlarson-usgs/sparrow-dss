...
public class DeliveryRunnerTest extends TestCase {
	...
	public void testDeliveryRunnerWithLinearFlow() {
...

		{
			...
			assertEquals(.01, result[0][0], TOLERANCE);
			assertEquals(1 , result[1][0], TOLERANCE);
		}
		
		{
			...
			double[][] result = dr.calculateDeliveryCoefficients(targetReaches);
			assertEquals("7 nodes returned", 7, result.length);
			assertEquals(.01 * .04 , result[0][0], TOLERANCE);
			assertEquals(.04 , result[1][0], TOLERANCE);
			assertEquals(1 , result[2][0], TOLERANCE);
		}
		
		{
			...
			double[][] result = dr.calculateDeliveryCoefficients(targetReaches);
			assertEquals("7 nodes returned", 7, result.length);
			assertEquals(.01 * .04 * .09, result[0][0], TOLERANCE);
			assertEquals(.04  * .09, result[1][0], TOLERANCE);
			assertEquals(.09 , result[2][0], TOLERANCE);
			assertEquals(1 , result[3][0], TOLERANCE);
		}
		
		{
			...
			double[][] result = dr.calculateDeliveryCoefficients(targetReaches);
			assertEquals("7 nodes returned", 7, result.length);
			assertEquals(.01 * .04 * .09 * .16, result[0][0], TOLERANCE);
			assertEquals(.04  * .09 * .16, result[1][0], TOLERANCE);
			assertEquals(.09 * .16, result[2][0], TOLERANCE);
			assertEquals(.16, result[3][0], TOLERANCE);
			assertEquals(1, result[4][0], TOLERANCE);
		}
