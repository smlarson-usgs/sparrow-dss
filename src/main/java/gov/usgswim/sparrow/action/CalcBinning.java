 ...
 * @author eeverman
 */
public class CalcBinning extends Action<BigDecimal[]> {
	...
	@Override
	public BigDecimal[] doAction() throws Exception {
		...
		switch(request.getBinType()) {
			case EQUAL_COUNT:
				return buildEqualCountBins(dataColumn.getTable(), dataColumn.getColumn(), request.getBinCount(), keepZeroes, keepExtremeValues);
			case EQUAL_RANGE:
				return getEqualRangeBins(dataColumn.getTable(), dataColumn.getColumn(), request.getBinCount(), false, true);
		}
	...
	public static BigDecimal[] buildEqualCountBins(Double[] sortedData, int binCount, Boolean useRounding) {
		...
		for (int i=1; i<binCount; i++) {
			...
			if (loIndex == hiIndex) {
				...
				bins[i] = round(value, loBound, hiBound);
				...
				if (Math.abs(valueIndex - loIndex) < Math.abs(hiIndex - valueIndex)) {
					...
					if (loIndex > 0) {
						...
						bins[i] = round((loBound + sortedData[hiIndex])/2, loBound, hiBound);
					} else {// Already at lowest, can't adjust
						double hiBound = perturbIfEqual(sortedData[hiIndex], sortedData[loIndex]);
						bins[i] = round(value, sortedData[loIndex], hiBound);
					}
					...
					if (hiIndex < totalRows - 1) {
						...
						bins[i] = round((hiBound + sortedData[loIndex])/2, sortedData[loIndex], hiBound);
					} else {// Already at lowest, can't adjust
						double hiBound = perturbIfEqual(sortedData[hiIndex], sortedData[loIndex]);
						bins[i] = round(value, sortedData[loIndex], hiBound);
					}
	 ...
	 */
	public static int getUniqueValueCount(Double[] sortedData, int maxCount) {
	...
	public static BigDecimal round(double value, double lo, double hi) {
		...
		for (BigDecimal sigfigs: CalcBinning.digitAccuracyMultipliers) {
			BigDecimal result = CalcBinning.findClosestInRange(value, lo, hi, exponent, sigfigs);
			if (result != null) {
