...
public class MapViewerSparrowDataProvider2
implements NSDataProvider
{
	public enum HANDLING_MODE {AS_XML_REQUEST, AS_CONTEXT_ID_REQUEST, AS_MODEL_ID_REQUEST, AS_RESTLIKE_PARAMETER_REQUEST};
	protected static Logger log =
		Logger.getLogger(MapViewerSparrowDataProvider2.class); //logging for this class
	...
	public MapViewerSparrowDataProvider2() {
	}
	...
	public NSDataSet buildDataSet(Hashtable<?,?> properties) {
		...
		PredictServiceRequest svsRequest = null;
		...
		HANDLING_MODE handling = determineHandling(properties);
		log.debug("Request treated " + handling.name());
		switch(handling) {
			case AS_CONTEXT_ID_REQUEST:
				// This is the main branch
				nsData = handleContextIDRequest(properties, startTime);
				return nsData;
			case AS_MODEL_ID_REQUEST:
				// TODO might be deprecated
				RunModeController.checkDeprecation();
				nsData = handleModelIDRequest(properties, startTime, nsData);
				return nsData;
			case AS_XML_REQUEST:
				// TODO might be deprecated
				RunModeController.checkDeprecation();
				try {
					svsRequest = parseServiceRequest(properties);
			...
			case AS_RESTLIKE_PARAMETER_REQUEST:
				// TODO might be deprecated
				RunModeController.checkDeprecation();
	...
	private PredictServiceRequest parseServiceRequest(Hashtable<?, ?> properties)
			throws Exception {
	...
	private NSDataSet handleContextIDRequest(Hashtable<?, ?> properties,
			long startTime) {
	...
	public HANDLING_MODE determineHandling(Hashtable<?, ?> properties) {
		if (properties.containsKey(XML_REQUEST_KEY) && properties.get(XML_REQUEST_KEY) != null) {
			return HANDLING_MODE.AS_XML_REQUEST;
		} else if (properties.containsKey(CONTEXT_ID) && properties.get(CONTEXT_ID) != null) {
			// This is the main case
			return HANDLING_MODE.AS_CONTEXT_ID_REQUEST;
		} else if (properties.containsKey(MODEL_ID) && properties.get(MODEL_ID) != null) {
			return HANDLING_MODE.AS_MODEL_ID_REQUEST;
		}
		return HANDLING_MODE.AS_RESTLIKE_PARAMETER_REQUEST;
	}
