...
public class DeliveryRunner2 implements Runner {
	/**
	...
	// CONSTRUCTORS
	// ============
	public DeliveryRunner2(DataTable topo, DataTable coef, DataTable src,
			DataTable decay) {
		this(new PredictDataImm(topo, coef, src, null, decay,
				null, null));
	}

	public DeliveryRunner2(PredictData data) {
...
//	order by sparrow_model_id, fnode, model_reach_id
	...
	public PredictResult calculateDeliveredFlux(PredictionContext2 context) throws Exception{

		PredictResult adjResult = CacheAvoider.avoidResultCache(context);
...

		{
			...
			if (analysis.isAggregated()) {
				...
				deliveryResult = WeightRunner2.doWeighting(context, deliveryResult);
			}
		}
