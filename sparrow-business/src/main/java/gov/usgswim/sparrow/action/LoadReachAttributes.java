...
public class LoadReachAttributes extends Action<DataTable> {
	...
	public DataTable doAction() throws Exception {
		...
		cols[13].setUnits(SparrowUnits.SQR_KM.getUserName());	//Total Contributing Area
		cols[13].setProperty(TableProperties.PRECISION.toString(), "2");
		...
		cols[15].setProperty(TableProperties.PRECISION.toString(), "6");
		cols[16].setUnits(SparrowUnits.FRACTION.getUserName());	//Split Fraction
		cols[16].setProperty(TableProperties.PRECISION.toString(), "2");
