...
public class CreateViewForLayer {
     ...
     * @throws IOException
     */
    public void createTableFromDbf(File dbfFile, PredictionContext context) throws IOException {
        ...
        String mydb = PostgresDAO.getDB_NAME_MODEL_OUTPUT();
        String dbfName = getDbfNameWithModelNbr(context);  //54n1663577716
        ...
        sb.append(", ");
        sb.append("\"shp2pgsql ");
        sb.append(dbfName); // can i pass a stream? else I may want to read the stream yuck 
        sb.append(".dbf ");
        sb.append("| psql -d ");
        sb.append(mydb); 
        sb.append("\""); 

        // also psql -d gisdatabase –U username –h hostname –p port -f parcels.sql will force a prompt for the pwd 
        ...
        // -k keep case ??, 
        ProcessBuilder pb = new ProcessBuilder(sb.toString());// ("/bin/sh", "-n", "-c", "shp2pgsql dbfName.dbf | psql -d mydb"); //will take a List<String> too
     ...
     * @param context All the state needed to run a prediction
     * @return base view name (without the catchment or flow)
     * @throws java.sql.SQLException
     */
    public String createModelOutputViews(PredictionContext context) throws SQLException {
        
        String viewName = null;


        createCatchmentView(context);
        createFlowView(context);
    ...
    private String createCatchmentView(PredictionContext context) throws SQLException {
        ...
        String rNetwork = MODEL_REGION.from(twoDigits).name() + "_catch"; //catch, flow or huc8 options 
        ...
        String sql = getCatchmentViewSql(viewLayerName, dbfTableName, rNetwork);
        LOGGER.info("CreateCatchementView sql:" + sql);
        LOGGER.info("___________________________________________");
        
        Connection connection = pgDao.getConnection();
        final PreparedStatement st = connection.prepareStatement(sql);

        try {
            st.execute(sql);  //this returns a boolean
        } finally {
            LOGGER.info("Closing connection CreateViewForLayer.createCatchmentView.");
            st.close();
        }
    ...
    private boolean createFlowView(PredictionContext context) {
        boolean result = false;

        return result;
    }

    private String getCatchmentViewSql(String viewLayerName, String dbfTableName, String riverNetworkTableName) {
        //#TODO# do null checks
        ...
        sql.append("net.geom ::geometry(MultiPolygon, 4326) As geom ");
        sql.append("FROM ");
        sql.append(PostgresDAO.getSCHEMA_NAME_SPARROW_OVERLAY());
        sql.append(".");
        ...
        LOGGER.debug("Attempting to create view of dbf with shape file : " + sql.toString());
...
//	return st.execute(sql.toString());}
    ...
    public enum MODEL_REGION {
        ...
        private MODEL_REGION(int value, String name) {
            this.value = value;
            this.name = name;
        }
    ...
    // map doesnt allow for easy translation of the numeric as a key - not used
//    public EnumMap<MODEL_REGION, String> MODEL_TO_REGION = new EnumMap<>(MODEL_REGION.class);
//
//    private void loadMap() {
//        for (MODEL_REGION model : MODEL_REGION.values()) {
//            this.MODEL_TO_REGION.put(model, model.name);
//        }
//    }
